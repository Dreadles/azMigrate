...
if ($curatedVmList.Length -ne 0) {
    ...
    foreach ($vm in $curatedVmList) {
        ...
        $valid = validateStates -allowedOperations $replicatedVm.AllowedOperation -processRequested @("Migrate")

        if ($valid) {

            try {
                $submittedJob = Start-AzMigrateServerMigration `
            ...
            Write-Host "$(get-date) - VM $($vm.TargetVMName) Start Migration Job Not Created - VM Not in a Valid State $($replicatedVm.TestMigrateState) and $($replicatedVm.MigrationState)" -ForegroundColor Red -BackgroundColor Black
        }  
    }
