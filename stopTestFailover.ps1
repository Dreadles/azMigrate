...
if ($curatedVmList.Length -ne 0) {
    ...
    foreach ($vm in $curatedVmList) {
        ...
        $valid = validateStates -allowedOperations $replicatedVm.AllowedOperation -processRequested @("TestMigrateCleanup")

        if ($valid) {

            try {
                $submittedJob = Start-AzMigrateTestMigrationCleanup `
            ...
            Write-Host "$(get-date) - VM $($vm.TargetVMName) Test Cleanup Job Not Created - VM Not in a Valid State $($replicatedVm.TestMigrateState) and $($replicatedVm.MigrationState)" -ForegroundColor Red -BackgroundColor Black
        }  
    }
