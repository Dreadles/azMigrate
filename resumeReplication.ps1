...
if ($curatedVmList.Length -ne 0) {
    ...
    foreach ($vm in $curatedVmList) {
        ...
        $valid = validateStates -allowedOperations $replicatedVm.AllowedOperation -processRequested @("ResumeReplication")

        if ($valid) {

            try {
                $submittedJob = Resume-AzMigrateServerReplication `
            ...
            Write-Host "$(get-date) - VM $($vm.TargetVMName) Resume Replication Job Not Created - VM Not in a Valid State $($replicatedVm.TestMigrateState) and $($replicatedVm.MigrationState) and $($replicatedVm.replicationStatus)" -ForegroundColor Red -BackgroundColor Black
        } 
    }
