...
if ($curatedVmList.Length -ne 0) {
    ...
    foreach ($vm in $curatedVmList) {
        ...
        try {
            $replicatedVm = Get-AzMigrateServerReplication -DiscoveredMachineId $vm.MachineId -ErrorAction SilentlyContinue
        }
        catch {
        ...
        if ($null -eq $replicatedVm) {
            Write-Host "$(Get-Date) - VM - $($vm.TargetVMName) is currently not replicating. Skipping" -ForegroundColor Yellow -BackgroundColor Black
        }
        Else {
            $valid = validateStates -allowedOperations $replicatedVm.AllowedOperation -processRequested @("DisableMigration")

            if ($valid) {
    
                try {
                    $submittedJob = Remove-AzMigrateServerReplication `
                ...
                Write-Host "$(get-date) - VM $($vm.TargetVMName) Stop Replication Job Not Created - VM Not in a Valid State $($replicatedVm.TestMigrateState) and $($replicatedVm.MigrationState) and $($replicatedVm.replicationStatus)" -ForegroundColor Red -BackgroundColor Black
            }  
        }
    }
